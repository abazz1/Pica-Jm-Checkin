...
class AutoTask:
    ...
    def get_host(self):
        ...
        """
        try:
            url = "https://sxsy.org/site.jpg"
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36 Edg/137.0.0.0',
            }
            response = requests.get(url, headers=headers)
            response.raise_for_status()
            text = self.ocr.classification(response.content)
            # OCR可能返回 Sxsy13com, sxsy13.com, sxsy13com 等格式
            text = text.lower().replace(' ', '')
            import re as re2
            m = re2.search(r'(sxsy\d+\.?com)', text)
            if m:
                ...
                return host
            logging.warning(f"[获取host]OCR无法识别域名，使用默认域名")
            return DEFAULT_HOST
        except Exception as e:
            logging.warning(f"[获取host]发生错误: {e}，使用默认域名")
            return DEFAULT_HOST
    ...
    def get_captcha_text(self, img_base64):
        ...
        try:
            ...
            logging.info(f"[获取验证码]识别结果: {result}")
            return result
        except Exception as e:
    ...
    def login_in(self, host, username, password, formhash, captcha, session, loginhash, seccodehash):
        ...
        try:
            ...
            payload = f"formhash={formhash}&referer=https://{host}/home.php?mod=spacecp&ac=credit&showcredit=1&loginfield={loginfield}&username={username}&password={password}&questionid=0&answer=&seccodehash={seccodehash}&seccodemodid=member::logging&seccodeverify={captcha}&cookietime=2592000"
            headers = {
    ...
    def solve_math_verify(self, text):
        """
        解析签到算术验证并返回答案
        :param text: 包含算术验证的响应文本
        :return: 答案数字或None
        """
        m = re.search(r'签到验证[：:]\s*(\d+)\s*([+\-xX*/])\s*(\d+)\s*=\s*\?', text)
        if not m:
            return None
        left, op, right = int(m.group(1)), m.group(2), int(m.group(3))
        if op == '+': ans = left + right
        elif op == '-': ans = left - right
        elif op in ('x', 'X', '*'): ans = left * right
        elif op == '/' and right != 0: ans = left // right
        else: return None
        return str(ans)
    ...
    def signin(self, host, session, sign_hash):
        ...
        try:
            ...
            # 检查是否需要算术验证
            answer = self.solve_math_verify(response.text)
            if answer:
                formhash2 = re.search(r'formhash=([a-zA-Z0-9]{8})', response.text)
                if formhash2:
                    sign_hash = formhash2.group(1)
                url2 = f"https://{host}/plugin.php?id=k_misign:sign&operation=qiandao&formhash={sign_hash}&format=global_usernav_extra&mathverify_answer={answer}"
            ...
            if match:
                ...
                if '今日已签' in text or '签到成功' in text:
                    logging.info("[签到]签到成功")
                else:
                    logging.warning(f"[签到]结果: {text}")
            else:
